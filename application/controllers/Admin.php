...
class Admin extends CI_Controller
{
	...
	public function tag_menagment()
	{
		try {
			...
			$crud->callback_read_field("rel_biblio", function ($value, $row) {
				...
				return "<a href='http://212.224.88.77/poetica/index.php/cruds/load_biblio_list/read/" .
					$row .
	...
	public function multi_upload()
	{
		try {
			...
			$crud->callback_after_insert([$this, "import_uploaded_file"]);
			$output = $crud->render();
	...
	{
		$file_to_process =
			getcwd() . "/assets/uploads/multi_upload/" . $data["file_path"];
		$fileMimes = [
...
<script type="text/javascript" src="' .
					base_url() .
					'/assets/js/xcolorpicker/xncolorpicker.min.js" ></script>
<script>
 ...
 var popupBasic = new Picker(parentBasic);
	...
	public function book_managment()
	{
		try {
			...
			$crud->callback_read_field("author_id", function ($value, $row) {
				$query = $this->db->query(
					"SELECT name, surname FROM author WHERE id=" . $value
				);
				$results = $query->result();
				$output = "";
				foreach ($results as $res) {
				...
				return "<a href='http://212.224.88.77/poetica/index.php/admin/author_managment/read/" .
					$value .
					"' target='blank'>$output</a>";
			});
			$crud->set_relation(
				"language_id",
				"languages",
			...
			$crud->callback_after_upload([$this, "check_json_book_to_process"]);
			$crud->callback_after_insert([$this, "insert_book_on_elastic"]);
			$crud->callback_after_update([$this, "update_book_on_elastic"]);
			$crud->callback_after_delete([$this, "delete_book_on_elastic"]);
	...
	public function check_json_book_to_process(
		$uploader_response,
		$field_info,
		$files_to_upload
	) {
		...
		if (
			$files_to_upload[$field_info->encrypted_field_name]["type"] ==
			"application/json"
		) {
