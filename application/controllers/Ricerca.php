...
class Ricerca extends CI_Controller {
   ...
   public function index($output = null, $path='ricerca/ricerca'){
    $languages = $this->db->query("SELECT * FROM `languages` WHERE id in (select distinct language_id as lan from book)");


        if ($this->session->userdata('currently_logged_in'))  {
            $this->template->load($this->session->userdata('user_info')['role'].'_layout', 'contents' , $path, ['languages'=>$languages]); 
        }
   }
