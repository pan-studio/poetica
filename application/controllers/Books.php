...
class Books extends CI_Controller
{
	...
	public function get_all_biblio_by_keyword()
	{
		...
		if ($this->session->userdata("currently_logged_in")) {
			echo json_encode(
				$this->book->get_all_biblio_by_keyword($_GET["keyword"])
			);
		}
	}
	...
	public function get_all_tags_by_keyword()
	{
		...
		if ($this->session->userdata("currently_logged_in")) {
			echo json_encode(
				$this->book->get_all_tags_by_keyword($_GET["keyword"])
			);
		}
	}
	...
	public function get_all_book_with_tag()
	{
		...
		if ($this->session->userdata("currently_logged_in")) {
			echo json_encode(
				$this->book->get_all_books_with_tag($_GET["keyword"])
			);
		}
	}
	...
	public function advanced_search()
	{
		...
		if ($this->session->userdata("currently_logged_in")) {
			...
			if ($foundSearchForTag == true) {
				$query =
					"select  author.name, author.surname, book.title, book.file_path, book.id as bid from book, tag_relationship, tags, author where book.author_id = author.id and tag_relationship.id_book = book.id and tag_relationship.id_tag = tags.id and  ";
				$queryTags =
					"select * from tag_relationship, tags where tag_relationship.id_tag = tags.id and ";
				$queryBiblio =
					"select * from biblio_relationship, biblio where biblio_relationship.biblio_id = biblio.id and ";
			}
			...
			for ($i = 0; $i < count($_POST["cerca_per"]); $i++) {
				...
				if ($_POST["cerca_per"][$i] == "lingua") {
					...
					if ($_POST["tipologia"][$i] == "contiene") {
						$query .=
							" ( book.language_id = languages.id && languages.language like '%" .
							$_POST["keyword"][$i] .
							"%') ";
					} else {
						$query .=
							" ( book.language_id = languages.id && languages.language = '" .
							$_POST["keyword"][$i] .
							"') ";
					}
				}
				...
				if (
					$_POST["cerca_per"][$i] == "testo_nella_traduzione" ||
			...
			if (count($elasticFields) > 0) {
				...
				$elasticQuery->query->query_string->query =
					"*" . ltrim($elasticValue) . "*";
				...
				foreach ($res->hits->hits as $elasticBook) {
					...
					$bookResult->language_id =
						$elasticBook->_source->language_id;
					$bookResult->year = $elasticBook->_source->year;
					$bookResult->processed_book = basename(
						$elasticBook->_source->file_path
					);
					$bookResult->name = $elasticBook->_source->author_name;
					$bookResult->surname =
						$elasticBook->_source->author_surname;
					$bookResult->bid = $elasticBook->_id;
					$bookResult->info = "";
					$bookResult->annotations =
						$elasticBook->_source->annotations;
			...
			if ($searchInDb == true) {
				...
				if ($foundSearchForTag == true) {
					$result[
						"tags_mysql_result"
					] = $this->book->get_all_tags_by_query($queryTags);
				} else {
				...
				$books = $result["book_mysql_result"]["book"];
				$queryBiblio =
					"select * from biblio_relationship, biblio where biblio_relationship.id_biblio = biblio.id and ";
				if (count($books) > 0) {
					foreach ($books as $book) {
						$queryBiblio .=
							" biblio_relationship.id_book = " .
							$book["bid"] .
							" OR";
					}
					...
					$result[
						"biblio_mysql_result"
					] = $this->book->get_all_biblio_by_query($queryTags);
				}
				...
				foreach ($elasticResult as $elRes) {
					array_push(
						$jsonSql["book"],
						json_decode(json_encode($elRes), true)
					);
			...
			for (
				$i = 0;
				$i < count($result["book_mysql_result"]["book"]);
				$i++
			) {
				array_push(
					$biblioIds,
					$result["book_mysql_result"]["book"][$i]["bid"]
				);
			}

			if (count($biblioIds) > 0) {
				$queryBiblio =
					"select biblio.* from biblio, biblio_relationship where biblio_relationship.id_biblio=biblio.id and ";
				foreach ($biblioIds as $biblioId) {
					$queryBiblio .=
						" biblio_relationship.id_book=" . $biblioId . " OR";
				}
				$queryBiblio = explode(" ", $queryBiblio);
				array_splice($queryBiblio, -1);
				...
				$result[
					"biblio_mysql_result"
				] = $this->book->get_all_biblio_by_query($queryBiblio);
			} else {
				$result["biblio_mysql_result"] = [];
			}

			$result["book_mysql_result"]["book"] = $this->super_unique(
				$result["book_mysql_result"]["book"],
				"id"
			);
			$result["tags_mysql_result"] = $this->super_unique(
				$result["tags_mysql_result"],
				"id"
			);
			$result["biblio_mysql_result"] = $this->super_unique(
				$result["biblio_mysql_result"],
				"id"
			);
	...
	public function find_advanced_books()
	{
		...
		if ($this->session->userdata("currently_logged_in")) {
			$role = $this->session->userdata("user_info")["role"];
			$query =
				"select *, book.id as bid from book left join author on book.author_id = author.id  where book.author_id = author.id and ";
	...
	public function find_books()
	{
		...
		if ($this->session->userdata("currently_logged_in")) {
			...
			for ($i = 0; $i < count($_POST["randomId"]); $i++) {
				...
				if (
					isset($_POST["century_from_" . $randId]) &&
					isset($_POST["century_to_" . $randId])
				) {
					$query .=
						" ((book.year div 100) + 1) >= " .
						$_POST["century_from_" . $randId] .
						" ";
					$query .=
						" and ((book.year div 100) + 1) <= " .
						$_POST["century_to_" . $randId] .
						" ";
				}

				if (
					isset($_POST["year_from_" . $randId]) &&
					isset($_POST["year_to_" . $randId])
				) {
					$query .=
						" ((book.year div 100) + 1) >= " .
						$_POST["year_from_" . $randId] .
						" ";
					$query .=
						" and ((book.year div 100) + 1) <= " .
						$_POST["year_to_" . $randId] .
						" ";
				}
				...
				if (isset($_POST["operator_language_" . $randId])) {
					if (isset($_POST["operator_" . $randId])) {
						$query .=
							" " .
							$_POST["operator_" . $randId] .
							" book.language_id = " .
							$_POST["language_" . $randId] .
							" ";
						...
						$query .=
							...
							$_POST["language_" . $randId] .
							" " .
							$_POST["operator_language_" . $randId];
					}
				} else {
