...
class ElasticSearch
{
    ...
    public function add($type, $id, $data, $method='PUT')
    {
        return $this -> call($type . '/' . $id, $method, $data);
    }
    ...
    public function queryCurl($type='GET', $q)
    {
        $ch = curl_init();

            curl_setopt($ch, CURLOPT_URL, 'http://212.224.88.77:9200/poetica/_doc/_search');
    ...
    public function updateAll($id, $method='POST', $data='')
    {
        ...
        return $this -> call('_update/'.$id, $method, $newData);
    }

    public function updateByField($id, $method='POST', $data='')
    {
      
       $newData = new stdClass();
        $newData->doc = new stdClass();
        /*$newData->doc->annotations = new stdClass();
        $newData->doc->annotations->rows = [];*/
        return $this -> call('_update/'.$id, $method, json_encode($data));
    }
}